...
class TestDownloadWithRetry:
    ...
    def test_retry_success_after_failures(self):
        ...
        mock_func = MagicMock(
            side_effect=[BrowserError("First fail"), BrowserError("Second fail"), "success"]
        )
    ...
    def test_retry_with_download_error(self):
        ...
        mock_func = MagicMock(side_effect=[DownloadError("First fail"), "success"])
    ...
    def test_retry_with_timeout_exception(self):
        ...
        mock_func = MagicMock(side_effect=[TimeoutException("Timeout"), "success"])
...
class TestConvertCsvToFormat:
    ...
    def test_convert_parquet_pyarrow_error(self, tmp_path):
        ...
        with patch(
            "pandas.DataFrame.to_parquet", side_effect=ImportError("No module named 'pyarrow'")
        ):
...
class TestDownloadGoogleTrendsCSVMocked:
    ...
    def test_element_click_intercepted_handling(self, mock_chrome):
        ...
        with patch("trendspyg.downloader.WebDriverWait") as mock_wait:
            mock_wait.return_value.until.side_effect = ElementClickInterceptedException(
                "Click blocked"
            )
    ...
    def test_successful_download_flow(
        self, mock_sleep, mock_listdir, mock_wait, mock_chrome, tmp_path
    ):
        ...
        with patch("trendspyg.downloader.os.path.join", return_value=str(csv_file)):
            with patch("trendspyg.downloader.os.rename"):
                result = download_google_trends_csv(geo="US", download_dir=str(tmp_path))
    ...
    def test_active_only_toggle(self, mock_sleep, mock_wait, mock_chrome, tmp_path):
        ...
        with patch("trendspyg.downloader.os.listdir", return_value=[]):
            with pytest.raises(DownloadError):
                download_google_trends_csv(geo="US", active_only=True, download_dir=str(tmp_path))
    ...
    def test_active_only_toggle_failure(self, mock_sleep, mock_chrome, tmp_path):
        ...
        with patch("trendspyg.downloader.WebDriverWait") as mock_wait:
            ...
            with patch("trendspyg.downloader.os.listdir", return_value=[]):
                with pytest.raises(DownloadError):
                    download_google_trends_csv(
                        geo="US", active_only=True, download_dir=str(tmp_path)
                    )
    ...
    def test_sort_info_message(self, mock_sleep, mock_wait, mock_chrome, tmp_path, capsys):
        ...
        with patch("trendspyg.downloader.os.listdir", return_value=[]):
            with pytest.raises(DownloadError):
                download_google_trends_csv(geo="US", sort_by="volume", download_dir=str(tmp_path))
    ...
    @patch("trendspyg.downloader.os.listdir")
    @patch("trendspyg.downloader.time.sleep")
    def test_dataframe_output_message(
        self, mock_sleep, mock_listdir, mock_wait, mock_chrome, tmp_path, capsys
    ):
        ...
        with patch("trendspyg.downloader.os.path.join", return_value=str(csv_file)):
            with patch("trendspyg.downloader.os.rename"):
                result = download_google_trends_csv(
                    geo="US", output_format="dataframe", download_dir=str(tmp_path)
                )
